...
def solve_implicit(ks, a, b, c, d, b_edge=None, d_edge=None):
    ...
    return solve_tridiag(a_tri, b_tri, c_tri, d_tri), water_mask, a_tri, b_tri, c_tri, d_tri
...
# @jax.jit
def integrate_tke_old(u, v, w, maskU, maskV, maskW, dxt, dxu, dyt, dyu, dzt, dzw, cost, cosu, kbot, kappaM, mxl, forc, forc_tke_surface, tke, dtke):
    tau = 0
    taup1 = 1
    taum1 = 2
    ...
    sol, water_mask, a_tri, b_tri, c_tri, d_tri = solve_implicit(ks, a_tri, b_tri, c_tri, d_tri, b_edge=b_tri_edge)
...
def run(*inputs, device='cpu'):
    outputs = integrate_tke_old(*inputs)
    for o in outputs:
        o.block_until_ready()
    return outputs
