...
if __name__ == "__main__":
    ...
    print("jax first call took: " + str((end - start)*1000) + "ms")
    ...
    print("jax second call took: " + str((end - start)*1000/200) + "ms")
    ...
    print("cupy first call took: " + str((end - start)*1000) + "ms")
    ...
    print("cupy second call took: " + str((end - start)*1000) + "ms")
    ...
    fun = jax.jit(superbee.superbee)

    start = time.time()
    # for i in range(10):
    jaxres1, jaxres2, jaxres3 = fun(jax_arg1, jax_arg2, jax_arg3, jax_arg4, jax_arg5, jax_arg6, jax_arg7, jax_arg8, jax_arg9, jax_arg10, jax_arg11)

    jaxres1.block_until_ready()
    jaxres2.block_until_ready()
    jaxres3.block_until_ready()
    end = time.time()
    print("jax cuda call took: " + str((end - start)*1000) + "ms")
    # np.testing.assert_allclose(jnp.asarray(jaxres1), res1)
    # np.testing.assert_allclose(jnp.asarray(jaxres2), res2)
    # np.testing.assert_allclose(jnp.asarray(jaxres3), res3)
    
    jax_arg1 = jnp.array(arg1, dtype=np.float)
    jax_arg2 = jnp.array(arg2, dtype=np.float)
    jax_arg3 = jnp.array(arg3, dtype=np.float)
    jax_arg4 = jnp.array(arg4, dtype=np.float)
    jax_arg5 = jnp.array(arg5, dtype=np.float)
    jax_arg6 = jnp.array(arg6, dtype=np.float)
    jax_arg7 = jnp.array(arg7, dtype=np.float)
    jax_arg8 = jnp.array(arg8, dtype=np.float)
    jax_arg9 = jnp.array(arg9, dtype=np.float)
    jax_arg10 = jnp.array(arg10, dtype=np.float)
    jax_arg11 = jnp.array(arg11, dtype=np.float)
    ...
    print("jax cuda call took: " + str((end - start)*1000/10) + "ms")
